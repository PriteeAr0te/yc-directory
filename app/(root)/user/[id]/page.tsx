...
const page = async ({ params }: { params: Promise<{ id: string }> }) => {
    ...
    return (
        <>
            <section className='w-full pb-10 pt-20 px-6 max-w-7xl justify-center mx-auto lg:flex-row flex-col flex gap-10'>
                <div className='w-80 px-6 pb-6 pt-20 flex flex-col justify-center items-center bg-primary border-[5px] border-black shadow-100 rounded-[30px] relative z-0 h-fit max-lg:w-full'>
                    ...
                    <p className='text-[30px] font-extrabold text-white mt-7 text-center'>@{user?.username}</p>
                    <p className='mt-1 text-center font-normal text-sm text-white-100/80'>{user?.bio}</p>
                </div>
